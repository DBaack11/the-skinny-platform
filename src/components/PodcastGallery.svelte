...
<main class="mt-12 mb-60 justify-self-center flex flex-col items-center justify-center w-[90%]">
  ...
  <div class="flex flex-col items-center justify-center w-full">
    ...
    {#if currentEpisode}
      <div id="video-player" class={`fixed z-[60] ${pictureInPicture ? videoPlayerClasses.pip : videoPlayerClasses.expanded}`}>
        <SquareX
          ...
          class="hover:text-skinny-orange transition-all duration-200 absolute cursor-pointer {pictureInPicture ? 'text-black -top-8 right-0' : 'text-white -top-10 right-0'}"
          onclick={(e) => handleCloseVideoPlayer(e)}
        />
        {#if pictureInPicture}
          <SquareArrowUpLeft
            size={30}
            class="text-black hover:text-skinny-orange transition-all duration-200 absolute -top-8 right-10 cursor-pointer"
        ...
        {#if !pictureInPicture}
          <p class="relative -bottom-10 left-1/2 -translate-x-1/2 text-left text-white text-sm lg:text-xl font-light drop-shadow-[0_0_2px_rgba(0,0,0,1)] overflow-y-auto w-[90%] lg:w-[80vw] max-h-[60vh] lg:max-h-[400px]">
    ...
    {#snippet podcastGallery(episodes)}
      <div class="mt-16 w-full">
        <!-- PODCAST GALLERY VIEW -->
        <div class="flex flex-row items-start justify-evenly flex-wrap gap-8">
          {#each episodes as episode}
            ...
            <div class="group flex flex-col items-center justify-center">
              <button class="flex flex-col items-center justify-center cursor-pointer rounded-lg" onclick={() => handleEpisodeClick(episode)}>
                ...
                <div class="relative rounded-xl w-[350px] lg:w-[400px] aspect-video overflow-hidden transition-all duration-200 {isActive ? activeClasses.card : hoverClasses.card}">
                  <!-- Gradient Overlay -->
                  <div
              ...
              <div class="self-start pl-1 cursor-pointer transition-all duration-200 {isActive ? activeClasses.titleContainer : hoverClasses.titleContainer}">
                <p class="text-lg font-medium word-wrap max-w-[350px] lg:max-w-[400px]">{episode.name}</p>
              </div>
            </div>
          {/each}
        </div>
      ...
      <div class="podcast-slider flex transition-all duration-500 ease-in-out" style="transform: translateX({currentPodcast === PODCAST_NAMES.SKINNY ? '-50%' : '0%'});">
        {#each [PODCAST_NAMES.EXPANDERS, PODCAST_NAMES.SKINNY] as podcast}
          <div class="podcast-gallery w-1/2 flex-shrink-0 px-[5vw] flex justify-center">
            {@render podcastGallery(podcasts[podcast])}
          </div>
        {/each}
      </div>
    </div>
